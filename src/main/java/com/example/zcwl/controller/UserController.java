...
// 作用：所有该控制器下的API接口都会以/api/users开头
// 例如：http://localhost:8080/api/users
...
public class UserController {
     ...
     * 构造函数
     * <p>
     * 这个构造函数用于将用户服务对象注入到控制器中
     * @param userService 用户服务对象，由Spring容器自动注入
     */
    // @Autowired：Spring注解，用于自动注入依赖
    // 作用：告诉Spring容器，当创建UserController对象时，自动将UserService的实例传递给这个构造函数
    @Autowired
     ...
     * 前端调用方式：
     * - 请求类型：POST
     * - 请求URL：http://localhost:8080/api/users
     * - 请求体：JSON格式的用户数据，例如：
     *   {
     *     "uId": "user001",
     *     "name": "张三",
    ...
    // 作用：当前端发送POST请求到/api/users时，会调用这个方法
    @PostMapping
