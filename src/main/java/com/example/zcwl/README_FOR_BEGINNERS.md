...
|----------------------------|------|---------|----------------|
| `/api/auth/register`       | POST | 用户注册    | 新用户注册时调用       |
| `/api/auth/login`          | POST | 用户登录    | 用户登录时调用        |
| `/api/auth/validate-token` | GET  | 验证token | 验证token是否有效时调用 |

#### 4.1.1 用户注册
- **请求 URL**: `http://localhost:8080/api/auth/register`
- **请求方法**: POST
- **请求体**:
  ```json
  {
...
#### 4.1.3 验证token
- **请求 URL**: `http://localhost:8080/api/auth/validate-token?token={token}`
- **请求方法**: GET
- **成功响应** (200 OK):
  ```json
  true
...
- **密码加密**：使用 BCryptPasswordEncoder 对密码进行加密存储，确保密码安全
- **密码强度**：建议前端实现密码强度检查，要求密码长度至少6位，包含字母和数字
