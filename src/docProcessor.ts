...
export class DocProcessor {
  ...
  async processDocument(docId: string, chapterId: string, content: string): Promise<ChunkVector[]> {
    console.log(`处理文档 ${docId} 章节 ${chapterId}...`);
    
    // 语义切片
    const chunks = await this.apiClient.aiSemanticChunking(content, MAX_CHUNK_SIZE);
    ...
    try {
      ...
      for (const doc of docList) {
        ...
        for (const chapter of docInfo.docDir) {
          try {
            const content = await this.apiClient.getDocContent(doc.id, chapter.id);
            const chunks = await this.processDocument(doc.id, chapter.id, content.content);
            allChunks.push(...chunks);
          } catch (error) {
            console.error(`处理章节 ${chapter.name} (ID: ${chapter.id}) 失败:`, error);
            continue;
          }
        }
      }
